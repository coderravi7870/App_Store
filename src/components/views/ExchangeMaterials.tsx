...
const ExchangeMaterials = () => {
    ...
    setPendingData(
        storeInSheet
            .filter((i) => 
                i.planned10 && i.planned10 !== '' && // Column BA (Planned 10) not null
                (!i.actual10 || i.actual10 === '')   // Column BB (Actual 10) null/empty
            )
            .map((i) => ({
...
useEffect(() => {
    setHistoryData(
        storeInSheet
            .filter((i) => 
                i.actual10 && i.actual10 !== '' // Column BB (Actual 10) not null
            )
            .map((i) => ({
                timestamp: i.timestamp || '',
                liftNumber: i.liftNumber || '',
        ...
        await postToSheet(
            storeInSheet
                ...
                .map((prev) => ({
                    ...
                    status: values.status, // This will store 'Yes' or 'No' in column BD (index 55)
                })),
            'update',
            'STORE IN'
        );
